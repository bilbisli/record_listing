...
function display_record_count()
{
	...
	# display_record_count_record_sum must be unique
	display_record_count_record_sum=$(series_sum "${records_amount[@]}")
	ret_status=$?

 	echo "The sum of records is: ${display_record_count_record_sum}"

	insert_log ${LOG_EVENT} ${LOG_EVENT} ${display_record_count_record_sum}
